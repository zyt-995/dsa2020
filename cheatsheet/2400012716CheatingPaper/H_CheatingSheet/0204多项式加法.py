...
for _ in range(n):
    ...
    poly1=[[a[2*i],a[2*i+1]] for i in range(len(a)//2-1)]
    poly2=[[b[2*i],b[2*i+1]] for i in range(len(b)//2-1)]
    poly1.sort(key= lambda x:x[1],reverse=True)
    poly2.sort(key= lambda x:x[1],reverse=True)
    poly3=poly1+poly2
    poly3.sort(key= lambda x:x[1],reverse=True)
    i=0
    while i<len(poly3)-1:
        i+=1
        if(poly3[i][1]==poly3[i-1][1]):
            poly3[i-1][0]+=poly3[i][0]
            poly3.pop(i)
            i-=1
    for item in poly3:
        if item[0]!=0:
