...
def getneighbour(pos):
    ...
    for item in dir:
        ...
        nw_x=ix+pos[0]
        nw_y=iy+pos[1]
        if gra[nw_x][nw_y]!='#' and color[nw_x][nw_y]=='white':
            nei.append([nw_x,nw_y])
            color[nw_x][nw_y]="gray"
    return nei
...
for _ in range(T):
    ...
    gra=[['#']*(c+5) for _ in range(r+5)]
    color=[['white']*(c+5) for _ in range(r+5)]
